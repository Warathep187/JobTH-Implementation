...
const VerifyPage = () => {
  const [verify] = useMutation(authGql.COMPANY_VERIFY_ACCOUNT);
  ...
  useEffect(() => {
    ...
    if (verifyId) {
      verify({
        ...
        onCompleted: (res) => {
          setStatusMessage(res.companyVerifyAccount.msg);
          setIsVerified(true);
        },
        onError: (error) => {
  ...
  const redirect = () => {
    if (isVerified) {
      router.replace("/auth/companies/login");
    } else {
      router.replace("/auth/companies/create");
    }
  };
